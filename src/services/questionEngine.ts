...
export function validateQuestions(data: unknown): { questions: Question[]; errors: string[] } {
  ...
  data.forEach((q, index) => {
    if (!q || typeof q !== 'object' || !['id', 'question', 'answer', 'standard', 'source'].every(key => typeof q[key] === 'string' && q[key].trim()) ||
...
export function createQuestionEngine(questions: Question[], usedIds: string[] = []) {
  ...
  const getQuestions = (f: Filters) => questions.filter(q => q.grade === f.grade && (!f.difficulty || q.difficulty === f.difficulty) &&
    inRange(q.weekIntroduced, f.range, f.asOf) && (f.unused === false || !used.has(q.id)) &&
