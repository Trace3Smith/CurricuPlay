...
import { createQuestionEngine, familyOf } from '../../services/questionEngine';
...
export function isBoardEligible(assignments: unknown, questions: Question[], filters: Filters): boolean {
  ...
  if (new Set(Object.values(ids).map(id => byId.has(id as string) ? familyOf(byId.get(id as string)!) : id)).size !== tiles.length) return false;
