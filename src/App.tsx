...
export default function App() {
  ...
  return <div className={`app ${active ? 'playing' : ''}`}>
    ...
    {state.screen === 'home' && <main className="home">
      ...
      {(!bank.questions.length || bank.questions.every(q => q.reviewStatus === 'pending')) && <p className="content-notice" role="status">{bank.errors.length ? `Question bank needs correction: ${bank.errors.join(' ')}` : bank.questions.length ? `${bank.questions.length} generated questions await teacher review. The pacing guide is imported; approve reviewed questions in the local bank to enable play.` : 'No approved local questions are available. Add reviewed curriculum-backed questions to enable play.'}</p>}
    </main>}
    {state.screen === 'setup' && <main className="setup">
      ...
      {!available && <div className="content-notice" role="status">No questions available for this selection. {bank.questions.length ? bank.questions.every(q => q.reviewStatus === 'pending') ? 'Generated questions are awaiting teacher review and approval in the local bank.' : 'Try another grade, date, or Everything Taught So Far.' : 'Add reviewed local questions to enable play.'}</div>}
