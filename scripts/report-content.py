...
with (ROOT/'content/reports/question-review.csv').open('w') as f:
 w=csv.writer(f,lineterminator='\n');w.writerow(['id','grade','subject','difficulty','question','answer','teacherSetup','reviewStatus','weekIntroduced','alignedWeeks','standard','session','curriculumId','evidence','sourceUrl'])
...
lines+=['', f'**Total: {len(qs)} drafts, {r["approvedQuestions"]} teacher-approved questions in source JSON.** Review reuses eligible questions from these subjects; it adds no duplicate records.', '',
 ...
 f'All {len(qs)} drafts require teacher review of accuracy, scope and difficulty. **{len(r["teacherCheckedOrMaterialsRequired"])} questions use a teacher-check rubric or classroom materials.** `teacherSetup` identifies maps, observations, samples or other necessary materials. The app displays these preparation instructions. A teacher-check answer is intentionally a rubric, not a fabricated universal answer. Review worksheet: [question-review.csv](question-review.csv).', '',
 '## Coverage and limits', '',
 f'Source-file decisions: {sum(q["reviewStatus"]=="approved" for q in qs)} approved / {sum(q["reviewStatus"]=="pending" for q in qs)} pending / {sum(q["reviewStatus"]=="rejected" for q in qs)} rejected. Browser-local decisions are separate. The new material metadata and replacement wording invalidate old fingerprints; those questions require re-review.', '',
...
for g in ['K','1','2','3','4','5']:
 count=sum(q['grade']==g and q['requiresExternalClassroomMaterial'] is False and any('2026-08-31'<=w<=r['asOf'] for w in q['alignedWeeks']) for q in qs)
 lines.append(f'| {g} | {count} | {sum(q["grade"]==g and q["requiresExternalClassroomMaterial"] is False for q in qs)} | {sum(q["grade"]==g and q["reviewStatus"]=="approved" and q["requiresExternalClassroomMaterial"] is False for q in qs)} |')
lines+=['', '| Grade | Range | Eligible approved | Short of 30 | Short of 60 | Extra drafts needed for 60 even if all approved |', '|---|---|---:|---:|---:|---:|']
for c in r['gradeCoverage']:
 lines.append(f"| {c['grade']} | {c['range']} | {c['eligibleApprovedUnique']} | {c['approvedShortfallForFullGame']} | {c['approvedShortfallForVariationTarget']} | {c['additionalDraftsNeededEvenIfAllApprovedForVariationTarget']} |")
lines+=['', 'Approval counts here reflect source JSON only; teacher decisions stored in a browser are shown in Question Review. No current draft was automatically approved.', '', 'The complete **grade × category × difficulty × range** matrix is in [validation.json](validation.json). `alignedWeeks` records explicitly reviewed recurring instruction while `weekIntroduced` remains the first mapped week. Recent Content checks aligned weeks; both modes exclude future introduction dates and future aligned weeks. Undated records never enter date filters.', '',
